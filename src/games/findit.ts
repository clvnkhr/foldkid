...
export const EmojiPackKey = S.Union([
  S.Literal('fun'), S.Literal('numbers'), S.Literal('animals'),
  S.Literal('food'), S.Literal('vehicles'), S.Literal('nature'),
])
export type EmojiPackKey = typeof EmojiPackKey.Type

export const EMOJI_PACKS: ReadonlyArray<{ key: EmojiPackKey; labelKey: StringKey; sample: string }> = [
  ...
  { key: 'food', labelKey: 'findItPackFood', sample: '🍎🍓🥕' },
  { key: 'vehicles', labelKey: 'findItPackVehicles', sample: '🚗🚂🚀' },
  { key: 'nature', labelKey: 'findItPackNature', sample: '☀️🌳🌊' },
]

export const DEFAULT_EMOJI_PACK_KEYS: EmojiPackKey[] = EMOJI_PACKS.map(({ key }) => key)
...
const EMOJIS = [
  ...
  ['🍎', 'Apple', 'food'], ['🍌', 'Banana', 'food'], ['🍓', 'Strawberry', 'food'], ['🍇', 'Grapes', 'food'],
  ['🍉', 'Watermelon', 'food'], ['🍒', 'Cherries', 'food'], ['🥕', 'Carrot', 'food'], ['🌽', 'Corn', 'food'],
  ['🥦', 'Broccoli', 'food'], ['🥨', 'Pretzel', 'food'], ['🍪', 'Cookie', 'food'], ['🍦', 'Ice Cream', 'food'],
  ['🚗', 'Car', 'vehicles'], ['🚌', 'Bus', 'vehicles'], ['🚕', 'Taxi', 'vehicles'], ['🚑', 'Ambulance', 'vehicles'],
  ['🚒', 'Fire Engine', 'vehicles'], ['🚲', 'Bicycle', 'vehicles'], ['🛴', 'Scooter', 'vehicles'], ['🚂', 'Train', 'vehicles'],
  ['✈️', 'Airplane', 'vehicles'], ['🚁', 'Helicopter', 'vehicles'], ['🚀', 'Rocket', 'vehicles'], ['⛵', 'Sailboat', 'vehicles'],
  ['☀️', 'Sun', 'nature'], ['🌙', 'Moon', 'nature'], ['☁️', 'Cloud', 'nature'], ['🌧️', 'Rain Cloud', 'nature'],
  ['⚡', 'Lightning', 'nature'], ['❄️', 'Snowflake', 'nature'], ['🌊', 'Wave', 'nature'], ['🔥', 'Fire', 'nature'],
  ['🌳', 'Tree', 'nature'], ['🌻', 'Sunflower', 'nature'], ['🍄', 'Mushroom', 'nature'], ['🌍', 'Earth', 'nature'],
] as const
...
export const EMOJI_NAMES_BY_LANG: Record<string, string[]> = {
  zh: ['气球', '派对炮', '五彩纸屑', '礼物', '泰迪熊', '棒棒糖', '糖果', '生日蛋糕', '彩虹', '樱花', '星星', '披萨', '汉堡', '玉米饼', '甜甜圈', '杯子蛋糕', '零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '猫', '狗', '兔子', '蝴蝶', '独角兽', '熊', '熊猫', '考拉', '狮子', '老虎', '青蛙', '猴子', '狐狸', '马', '浣熊', '牛', '猪', '章鱼', '企鹅', '鸟', '鹰', '猫头鹰', '小鸡', '鸭子', '乌龟', '蛇', '蜥蜴', '鳄鱼', '鲸鱼', '海豚', '鲨鱼', '鱼', '河豚', '蜜蜂', '瓢虫', '蟋蟀', '蚂蚁', '苹果', '香蕉', '草莓', '葡萄', '西瓜', '樱桃', '胡萝卜', '玉米', '西兰花', '椒盐卷饼', '饼干', '冰淇淋', '汽车', '公交车', '出租车', '救护车', '消防车', '自行车', '滑板车', '火车', '飞机', '直升机', '火箭', '帆船', '太阳', '月亮', '云', '雨云', '闪电', '雪花', '海浪', '火', '树', '向日葵', '蘑菇', '地球'],
  fr: ['Ballon', 'Coton', 'Confetti', 'Cadeau', 'Ours en peluche', 'Sucette', 'Bonbon', 'Gâteau d\'anniversaire', 'Arc-en-ciel', 'Fleur de cerisier', 'Étoile', 'Pizza', 'Burger', 'Taco', 'Donut', 'Petit gâteau', 'Zéro', 'Un', 'Deux', 'Trois', 'Quatre', 'Cinq', 'Six', 'Sept', 'Huit', 'Neuf', 'Chat', 'Chien', 'Lapin', 'Papillon', 'Licorne', 'Ours', 'Panda', 'Koala', 'Lion', 'Tigre', 'Grenouille', 'Singe', 'Renard', 'Cheval', 'Raton laveur', 'Vache', 'Cochon', 'Pieuvre', 'Manchot', 'Oiseau', 'Aigle', 'Hibou', 'Poussin', 'Canard', 'Tortue', 'Serpent', 'Lézard', 'Crocodile', 'Baleine', 'Dauphin', 'Requin', 'Poisson', 'Poisson-globe', 'Abeille', 'Coccinelle', 'Grillon', 'Fourmi', 'Pomme', 'Banane', 'Fraise', 'Raisins', 'Pastèque', 'Cerises', 'Carotte', 'Maïs', 'Brocoli', 'Bretzel', 'Biscuit', 'Glace', 'Voiture', 'Bus', 'Taxi', 'Ambulance', 'Camion de pompiers', 'Vélo', 'Trottinette', 'Train', 'Avion', 'Hélicoptère', 'Fusée', 'Voilier', 'Soleil', 'Lune', 'Nuage', 'Nuage de pluie', 'Éclair', 'Flocon de neige', 'Vague', 'Feu', 'Arbre', 'Tournesol', 'Champignon', 'Terre'],
  de: ['Ballon', 'Partyknaller', 'Konfetti', 'Geschenk', 'Teddybär', 'Lutscher', 'Bonbon', 'Geburtstagskuchen', 'Regenbogen', 'Kirschblüte', 'Stern', 'Pizza', 'Burger', 'Taco', 'Donut', 'Törtchen', 'Null', 'Eins', 'Zwei', 'Drei', 'Vier', 'Fünf', 'Sechs', 'Sieben', 'Acht', 'Neun', 'Katze', 'Hund', 'Hase', 'Schmetterling', 'Einhorn', 'Bär', 'Panda', 'Koala', 'Löwe', 'Tiger', 'Frosch', 'Affe', 'Fuchs', 'Pferd', 'Waschbär', 'Kuh', 'Schwein', 'Krake', 'Pinguin', 'Vogel', 'Adler', 'Eule', 'Küken', 'Ente', 'Schildkröte', 'Schlange', 'Eidechse', 'Krokodil', 'Wal', 'Delfin', 'Hai', 'Fisch', 'Kugelfisch', 'Biene', 'Marienkäfer', 'Grille', 'Ameise', 'Apfel', 'Banane', 'Erdbeere', 'Trauben', 'Wassermelone', 'Kirschen', 'Karotte', 'Mais', 'Brokkoli', 'Brezel', 'Keks', 'Eis', 'Auto', 'Bus', 'Taxi', 'Krankenwagen', 'Feuerwehrauto', 'Fahrrad', 'Roller', 'Zug', 'Flugzeug', 'Hubschrauber', 'Rakete', 'Segelboot', 'Sonne', 'Mond', 'Wolke', 'Regenwolke', 'Blitz', 'Schneeflocke', 'Welle', 'Feuer', 'Baum', 'Sonnenblume', 'Pilz', 'Erde'],
  fa: ['بادکنک', 'ترقه', 'کاغذ رنگی', 'هدیه', 'خرس عروسکی', 'آبنبات چوبی', 'آبنبات', 'کیک تولد', 'رنگین‌کمان', 'شکوفه گیلاس', 'ستاره', 'پیتزا', 'برگر', 'تاکو', 'دونات', 'کاپ‌کیک', 'صفر', 'یک', 'دو', 'سه', 'چهار', 'پنج', 'شش', 'هفت', 'هشت', 'نه', 'گربه', 'سگ', 'خرگوش', 'پروانه', 'تک‌شاخ', 'خرس', 'پاندا', 'کوآلا', 'شیر', 'ببر', 'قورباغه', 'میمون', 'روباه', 'اسب', 'راکون', 'گاو', 'خوک', 'هشت‌پا', 'پنگوئن', 'پرنده', 'عقاب', 'جغد', 'جوجه', 'اردک', 'لاک‌پشت', 'مار', 'مارمولک', 'تمساح', 'نهنگ', 'دلفین', 'کوسه', 'ماهی', 'ماهی بادکنکی', 'زنبور', 'کفشدوزک', 'جیرجیرک', 'مورچه', 'سیب', 'موز', 'توت‌فرنگی', 'انگور', 'هندوانه', 'گیلاس', 'هویج', 'ذرت', 'کلم بروکلی', 'چوب‌شور', 'بیسکویت', 'بستنی', 'ماشین', 'اتوبوس', 'تاکسی', 'آمبولانس', 'ماشین آتش‌نشانی', 'دوچرخه', 'اسکوتر', 'قطار', 'هواپیما', 'بالگرد', 'موشک', 'قایق بادبانی', 'خورشید', 'ماه', 'ابر', 'ابر بارانی', 'صاعقه', 'دانه برف', 'موج', 'آتش', 'درخت', 'آفتابگردان', 'قارچ', 'زمین'],
  ms: ['Belon', 'Perapi', 'Konfeti', 'Hadiah', 'Beruang Teddy', 'Lolipop', 'Gula-gula', 'Kek Hari Jadi', 'Pelangi', 'Bunga Sakura', 'Bintang', 'Piza', 'Burger', 'Tako', 'Donut', 'Kek Cawan', 'Sifar', 'Satu', 'Dua', 'Tiga', 'Empat', 'Lima', 'Enam', 'Tujuh', 'Lapan', 'Sembilan', 'Kucing', 'Anjing', 'Arnab', 'Rama-rama', 'Unikorn', 'Beruang', 'Panda', 'Koala', 'Singa', 'Harimau', 'Katak', 'Monyet', 'Rubah', 'Kuda', 'Rakon', 'Lembu', 'Babi', 'Gurita', 'Penguin', 'Burung', 'Helang', 'Burung Hantu', 'Anak Ayam', 'Itik', 'Kura-kura', 'Ular', 'Cicak', 'Buaya', 'Ikan Paus', 'Ikan Lumba-lumba', 'Jerung', 'Ikan', 'Ikan Buntal', 'Lebah', 'Kumbang Kura-kura', 'Cengkerik', 'Semut', 'Epal', 'Pisang', 'Strawberi', 'Anggur', 'Tembikai', 'Ceri', 'Lobak merah', 'Jagung', 'Brokoli', 'Pretzel', 'Biskut', 'Ais krim', 'Kereta', 'Bas', 'Teksi', 'Ambulans', 'Kereta bomba', 'Basikal', 'Skuter', 'Kereta api', 'Kapal terbang', 'Helikopter', 'Roket', 'Perahu layar', 'Matahari', 'Bulan', 'Awan', 'Awan hujan', 'Kilat', 'Emping salji', 'Ombak', 'Api', 'Pokok', 'Bunga matahari', 'Cendawan', 'Bumi'],
  'zh-HK': ['氣球', '派對炮', '五彩紙屑', '禮物', '啤啤熊', '棒棒糖', '糖果', '生日蛋糕', '彩虹', '櫻花', '星星', '薄餅', '漢堡', '墨西哥卷', '冬甩', '紙杯蛋糕', '零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '貓', '狗', '兔仔', '蝴蝶', '獨角獸', '熊', '熊貓', '樹熊', '獅子', '老虎', '青蛙', '馬騮', '狐狸', '馬', '浣熊', '牛', '豬', '八爪魚', '企鵝', '雀仔', '鷹', '貓頭鷹', '雞仔', '鴨', '龜', '蛇', '蜥蜴', '鱷魚', '鯨魚', '海豚', '鯊魚', '魚', '雞泡魚', '蜜蜂', '瓢蟲', '蟋蟀', '蟻', '蘋果', '香蕉', '士多啤梨', '提子', '西瓜', '車厘子', '紅蘿蔔', '粟米', '西蘭花', '蝴蝶結餅', '餅乾', '雪糕', '汽車', '巴士', '的士', '白車', '消防車', '單車', '滑板車', '火車', '飛機', '直升機', '火箭', '帆船', '太陽', '月亮', '雲', '雨雲', '閃電', '雪花', '海浪', '火', '樹', '向日葵', '蘑菇', '地球'],
  ja: ['ふうせん', 'クラッカー', 'かみふぶき', 'プレゼント', 'くまのぬいぐるみ', 'ペロペロキャンディ', 'キャンディ', 'バースデーケーキ', 'にじ', 'さくら', 'ほし', 'ピザ', 'ハンバーガー', 'タコス', 'ドーナツ', 'カップケーキ', 'ゼロ', 'いち', 'に', 'さん', 'よん', 'ご', 'ろく', 'なな', 'はち', 'きゅう', 'ねこ', 'いぬ', 'うさぎ', 'ちょうちょ', 'ユニコーン', 'くま', 'パンダ', 'コアラ', 'ライオン', 'とら', 'かえる', 'さる', 'きつね', 'うま', 'アライグマ', 'うし', 'ぶた', 'たこ', 'ペンギン', 'とり', 'わし', 'ふくろう', 'ひよこ', 'あひる', 'かめ', 'へび', 'とかげ', 'わに', 'くじら', 'いるか', 'さめ', 'さかな', 'ふぐ', 'はち', 'てんとうむし', 'こおろぎ', 'あり', 'りんご', 'バナナ', 'いちご', 'ぶどう', 'すいか', 'さくらんぼ', 'にんじん', 'とうもろこし', 'ブロッコリー', 'プレッツェル', 'クッキー', 'アイスクリーム', 'くるま', 'バス', 'タクシー', 'きゅうきゅうしゃ', 'しょうぼうしゃ', 'じてんしゃ', 'キックボード', 'でんしゃ', 'ひこうき', 'ヘリコプター', 'ロケット', 'ヨット', 'たいよう', 'つき', 'くも', 'あまぐも', 'かみなり', 'ゆきのけっしょう', 'なみ', 'ひ', 'き', 'ひまわり', 'きのこ', 'ちきゅう'],
}
