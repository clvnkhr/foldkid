...
import { T as pipeArguments, _ as identity, p as dual } from "./Function-CAyx_go8.js";
import { none, some } from "./effect_Option.js";
import { H as fromIterable, dt as isArrayNonEmpty } from "./Array-DYbb2clh.js";
import { c as get } from "./Context-Bps2a48y.js";
import { gr as makeLatchUnsafe, hr as makeLatch } from "./effect-WSSJVTeC.js";
import { interrupt, succeed } from "./effect_Exit.js";
import { $ as doneUnsafe, Z as _await$1, et as interruptWith, nt as makeUnsafe$1 } from "./Layer-B4bJs9WE.js";
import { Scope, addFinalizer, addFinalizerExit, close as close$1, forkUnsafe, makeUnsafe as makeUnsafe$2 } from "./effect_Scope.js";
import { Gr as void_, Kr as when, Rr as uninterruptible, V as contextWith, Yt as map, Zr as withFiber, d as andThen, dr as succeed$1, ft as flatMap, g as as, gn as onInterrupt, gr as tap, hr as sync, jt as interrupt$1, mr as suspend, v as asVoid, yt as forEach } from "./Effect-X-nh5NFG.js";
import { nextPow2 } from "./effect_Number.js";
import { c as make$2, d as take$1, f as takeAll$1, l as prepend, r as append, s as filter, t as Empty, u as remove } from "./MutableList-CkGmI5pN.js";
import { i as set, n as get$1, r as make$3 } from "./MutableRef-0h-dUBFH.js";
//#region node_modules/effect/dist/Latch.js
var Latch_exports = /* @__PURE__ */ __exportAll({
	await: () => _await,
	close: () => close,
...
* @since 3.8.0
*/
var makeUnsafe = makeLatchUnsafe;
/**
* Creates a new Latch.
*
* **Previously Known As**
...
* @since 4.0.0
*/
var openUnsafe = (self) => self.openUnsafe();
/**
...
* @since 4.0.0
*/
var closeUnsafe = (self) => self.closeUnsafe();
/**
...
//#endregion
//#region node_modules/effect/dist/PubSub.js
/**
...
* @category constructors
*/
var make = (options) => sync(() => makePubSubUnsafe(options.atomicPubSub(), /* @__PURE__ */ new Map(), makeUnsafe$2(), makeUnsafe(false), make$3(false), options.strategy()));
/**
...
* @category constructors
*/
var makeAtomicUnbounded = (options) => new UnboundedPubSub(options?.replay ? new ReplayBuffer(options.replay) : void 0);
/**
...
* @category getters
*/
var size = (self) => sync(() => sizeUnsafe(self));
/**
* Retrieves the size of the queue, which is equal to the number of elements
* in the queue. This may be negative if fibers are suspended waiting for
* elements to be added to the queue.
...
*/
var sizeUnsafe = (self) => {
	if (get$1(self.shutdownFlag)) return 0;
...
* @category predicates
*/
var isFull = (self) => map(size(self), (size) => size === self.pubsub.capacity);
/**
...
* @category predicates
*/
var isEmpty = (self) => map(size(self), (size) => size === 0);
/**
...
* @category lifecycle
*/
var shutdown = (self) => uninterruptible(withFiber((fiber) => {
	set(self.shutdownFlag, true);
	return close$1(self.scope, interrupt(fiber.id)).pipe(andThen(self.strategy.shutdown), when(self.shutdownHook.open), asVoid);
}));
/**
...
* @category predicates
*/
var isShutdown = (self) => sync(() => isShutdownUnsafe(self));
/**
* Returns `true` if `shutdown` has been called, otherwise returns `false`.
*
* @example
...
* @category lifecycle
*/
var awaitShutdown = (self) => self.shutdownHook.await;
/**
...
* @category publishing
*/
var publish = /*#__PURE__*/ dual(2, (self, value) => suspend(() => {
	if (self.shutdownFlag.current) return succeed$1(false);
...
* @category publishing
*/
var publishUnsafe = /*#__PURE__*/ dual(2, (self, value) => {
...
* @category publishing
*/
var publishAll = /*#__PURE__*/ dual(2, (self, elements) => suspend(() => {
	if (self.shutdownFlag.current) return succeed$1(false);
	const surplus = self.pubsub.publishAll(elements);
	self.strategy.completeSubscribersUnsafe(self.pubsub, self.subscribers);
	if (surplus.length === 0) return succeed$1(true);
...
*/
var subscribe = (self) => uninterruptible(contextWith((services) => {
	...
	return addFinalizer(scope, unsubscribe(subscription)).pipe(andThen(addFinalizerExit(localScope, (exit) => close$1(scope, exit))), as(subscription));
}));
var unsubscribe = (self) => uninterruptible(withFiber((state) => {
	set(self.shutdownFlag, true);
	return forEach(takeAll$1(self.pollers), (d) => interruptWith(d, state.id), {
		discard: true,
...
* @category subscription
*/
var take = (self) => suspend(() => {
	if (self.shutdownFlag.current) return interrupt$1;
	if (self.replayWindow.remaining > 0) return succeed$1(self.replayWindow.take());
	const message = self.pollers.length === 0 ? self.subscription.poll() : Empty;
	if (message === Empty) return pollForItem(self);
	else {
		self.strategy.onPubSubEmptySpaceUnsafe(self.pubsub, self.subscribers);
...
* @category subscription
*/
var takeAll = (self) => suspend(function loop(value) {
	if (self.shutdownFlag.current) return interrupt$1;
	let as = self.pollers.length === 0 ? self.subscription.pollUpTo(Number.POSITIVE_INFINITY) : [];
	if (value) as = value.concat(as);
	self.strategy.onPubSubEmptySpaceUnsafe(self.pubsub, self.subscribers);
	if (self.replayWindow.remaining > 0) return succeed$1(self.replayWindow.takeAll().concat(as));
	else if (!isArrayNonEmpty(as)) return flatMap(pollForItem(self), (item) => loop([item]));
	return succeed$1(as);
});
...
* @category subscription
*/
var takeUpTo = /*#__PURE__*/ dual(2, (self, max) => suspend(() => {
	if (self.shutdownFlag.current) return interrupt$1;
	let replay = void 0;
	if (self.replayWindow.remaining >= max) return succeed$1(self.replayWindow.takeN(max));
	...
	self.strategy.onPubSubEmptySpaceUnsafe(self.pubsub, self.subscribers);
	return replay ? succeed$1(replay.concat(as)) : succeed$1(as);
}));
/**
...
* @category subscription
*/
var takeBetween = /*#__PURE__*/ dual(3, (self, min, max) => suspend(() => takeRemainderLoop(self, min, max, [])));
var takeRemainderLoop = (self, min, max, acc) => {
	if (max < min) return succeed$1(acc);
	return flatMap(takeUpTo(self, max), (bs) => {
		acc.push(...bs);
		const remaining = min - bs.length;
		if (remaining === 1) return map(take(self), (b) => {
			acc.push(b);
			return acc;
		});
		if (remaining > 1) return flatMap(take(self), (b) => {
			acc.push(b);
...
* @category getters
*/
var remaining = (self) => suspend(() => self.shutdownFlag.current ? interrupt$1 : succeed$1(self.subscription.size() + self.replayWindow.remaining));
/**
* Returns the number of messages currently available in the subscription.
*
* @example
...
* @category getters
*/
var remainingUnsafe = (self) => {
	if (self.shutdownFlag.current) return none();
	return some(self.subscription.size() + self.replayWindow.remaining);
};
...
var makeSubscriptionUnsafe = (pubsub, subscribers, strategy) => new SubscriptionImpl(pubsub, subscribers, pubsub.subscribe(), make$2(), makeUnsafe(false), make$3(false), strategy, pubsub.replayWindow());
...
var BoundedPubSubArbSubscription = class {
	...
	poll() {
		if (this.unsubscribed) return Empty;
...
var makePubSubUnsafe = (pubsub, subscribers, scope, shutdownHook, shutdownFlag, strategy) => new PubSubImpl(pubsub, subscribers, scope, shutdownHook, shutdownFlag, strategy);
...
var BackPressureStrategy = class {
	publishers = /*#__PURE__*/ make$2();
	get shutdown() {
		return withFiber((fiber) => forEach(takeAll$1(this.publishers), ([_, deferred, last]) => last ? interruptWith(deferred, fiber.id) : void_, {
			concurrency: "unbounded",
			discard: true
		}));
	}
	handleSurplus(pubsub, subscribers, elements, isShutdown) {
		return suspend(() => {
			...
			return (get$1(isShutdown) ? interrupt$1 : _await$1(deferred)).pipe(onInterrupt(() => {
	...
	onPubSubEmptySpaceUnsafe(pubsub, subscribers) {
		...
		while (keepPolling && !pubsub.isFull()) {
			const publisher = take$1(this.publishers);
			if (publisher === Empty) keepPolling = false;
			else {
				const [value, deferred] = publisher;
				const published = pubsub.publish(value);
				if (published && publisher[2]) doneUnsafe(deferred, succeed(true));
...
var strategyCompletePollersUnsafe = (strategy, pubsub, subscribers, subscription, pollers) => {
	...
	while (keepPolling && !subscription.isEmpty()) {
		...
		if (poller === Empty) {
			...
			if (pollResult === Empty) prepend(pollers, poller);
			else {
				doneUnsafe(poller, succeed(pollResult));
...
//#endregion
export { unbounded as A, sizeUnsafe as C, takeAll as D, take as E, make$1 as M, makeUnsafe as N, takeBetween as O, size as S, subscribe as T, publishAll as _, awaitShutdown as a, remainingUnsafe as b, dropping as c, isShutdown as d, isShutdownUnsafe as f, publish as g, makeAtomicUnbounded as h, SlidingStrategy as i, Latch_exports as j, takeUpTo as k, isEmpty as l, makeAtomicBounded as m, DroppingStrategy as n, bounded as o, make as p, PubSub_exports as r, capacity as s, BackPressureStrategy as t, isFull as u, publishUnsafe as v, sliding as w, shutdown as x, remaining as y };

//# sourceMappingURL=PubSub-CCYPM6Ux.js.map