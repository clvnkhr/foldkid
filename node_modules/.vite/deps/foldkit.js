...
var animationFrame = (config) => ({
	...
	dependenciesToStream: ({ isActive }) => when$1(makeAnimationFrameStream(config.toMessage), sync(() => isActive))
});
//#endregion
...
var view$15 = defineView((model, viewInputs) => {
	...
	const transitionAttributes = value(transitionState).pipe(when("EnterStart", () => [
		h.DataAttribute("closed", ""),
		h.DataAttribute("enter", ""),
		h.DataAttribute("transition", "")
	]), when("EnterAnimating", () => [h.DataAttribute("enter", ""), h.DataAttribute("transition", "")]), when("LeaveStart", () => [h.DataAttribute("leave", ""), h.DataAttribute("transition", "")]), when("LeaveAnimating", () => [
		h.DataAttribute("closed", ""),
...
var resolveNavigationKey = (key, isShift, focused, firstDayOfWeek) => value(key).pipe(withReturnType(), when("ArrowLeft", () => [
	addDays(focused, -1),
	-1,
	DAY_SKIP_CAP
]), when("ArrowRight", () => [
...
* (±12) navigation. */
var resolveMonthsKey = (key) => value(key).pipe(withReturnType(), when("ArrowLeft", () => -1), when("ArrowRight", () => 1), when("ArrowUp", () => -MONTHS_GRID_COLUMNS), when("ArrowDown", () => MONTHS_GRID_COLUMNS), when("PageUp", () => -MONTHS_IN_YEAR), when("PageDown", () => MONTHS_IN_YEAR), option);
...
var resolveYearsKey = (key) => value(key).pipe(withReturnType(), when("ArrowLeft", () => -1), when("ArrowRight", () => 1), when("ArrowUp", () => -YEARS_GRID_COLUMNS), when("ArrowDown", () => YEARS_GRID_COLUMNS), when("PageUp", () => -12), when("PageDown", () => YEARS_PAGE_SIZE), option);
...
var update$14 = (model, message) => value(message).pipe(withUpdateReturn$7, tagsExhaustive({
	...
	PressedKeyOnGrid: ({ key, isShift }) => value(model.viewMode).pipe(withUpdateReturn$7, when("Days", () => {
	...
	}), when("Months", () => match(resolveMonthsKey(key), {
		onNone: () => [
			model,
			[],
			none()
	...
	})), when("Years", () => match(resolveYearsKey(key), {
		onNone: () => [
			model,
			[],
			none()
	...
	ClickedHeading: () => value(model.viewMode).pipe(withUpdateReturn$7, when("Days", () => [
...
var view$13 = defineView((model, viewInputs) => viewInputs.toView(value(model.viewMode).pipe(withReturnType(), when("Days", () => buildDaysAttributes(model, viewInputs)), when("Months", () => buildMonthsAttributes(model, viewInputs)), when("Years", () => buildYearsAttributes(model, viewInputs)), exhaustive)));
//#endregion
...
var view$12 = defineView((model, viewInputs) => {
	...
	const animationAttributes = value(transitionState).pipe(when("EnterStart", () => [
		h.DataAttribute("closed", ""),
		h.DataAttribute("enter", ""),
		h.DataAttribute("transition", "")
	]), when("EnterAnimating", () => [h.DataAttribute("enter", ""), h.DataAttribute("transition", "")]), when("LeaveStart", () => [h.DataAttribute("leave", ""), h.DataAttribute("transition", "")]), when("LeaveAnimating", () => [
	...
	const handleButtonKeyDown = (key) => value(key).pipe(whenOr("Enter", " ", "ArrowDown", () => some(isOpen ? RequestedClose$1() : RequestedOpen$1())), when("Escape", () => when$2(isOpen, RequestedClose$1())), orElse$1(() => none()));
	...
	const handlePanelKeyDown = (key) => value(key).pipe(when("Escape", () => some(RequestedClose$1())), orElse$1(() => none()));
	const buttonAttributes = [
		h.Id(`${id}-button`),
		h.Type("button"),
...
var view$10 = defineView((model, viewInputs) => {
	...
	const handleKeyUp = (key) => value(key).pipe(when(" ", () => some(Toggled$2())), orElse$1(() => none()));
...
var makeView = (behavior) => {
	const impl = defineView((model, viewInputs) => {
		...
		const handleInputKeyDown = (key) => value(key).pipe(when("ArrowDown", () => {
...
var resolveKeyboardMoveTarget = (config) => sync(() => value(config.direction).pipe(withReturnType(), whenOr("Down", "Right", () => resolveWithinContainer({
	...
	currentIndex: config.currentIndex,
	isForward: false
})), when("NextContainer", () => resolveBetweenContainers({
	currentContainerId: config.currentContainerId,
	isForward: true
})), when("PreviousContainer", () => resolveBetweenContainers({
	currentContainerId: config.currentContainerId,
	isForward: false
})), exhaustive));
	...
	return flatMap(pipe(document.elementsFromPoint(clientX, clientY), fromIterable, findFirst((element) => element.hasAttribute("data-droppable-id"))), (container) => {
		...
		return some({
			...
			index: pipe(sortableItems, findFirstIndex((item) => {
				...
				return value(orientation).pipe(when("Vertical", () => clientY < rect.top + rect.height / 2), when("Horizontal", () => clientX < rect.left + rect.width / 2), exhaustive);
...
var subscriptions$1 = make$3()((entry) => ({
	documentPointer: entry({
		...
		dependenciesToStream: ({ dragActivity, orientation }) => {
			return when$1(merge(merge(fromEventListener(document, "pointermove").pipe(mapEffect((event) => sync(() => MovedPointer({
	...
	documentEscape: entry({ dragActivity: DragActivity }, {
		...
		dependenciesToStream: ({ dragActivity }) => when$1(fromEventListener(document, "keydown").pipe(filter$2(({ key }) => key === "Escape"), map$4(() => CancelledDrag())), sync(() => dragActivity === "Active"))
	}),
	documentKeyboard: entry({ dragActivity: KeyboardDragActivity }, {
		...
		dependenciesToStream: ({ dragActivity }) => when$1(fromEventListener(document, "keydown").pipe(mapEffect((event) => sync(() => {
	...
	autoScroll: entry({
		...
		dependenciesToStream: ({ isDragging }, readDependencies) => when$1(callback$1((queue) => acquireRelease(sync(() => {
			const ref = { id: 0 };
			const step = () => {
...
var arrowKeyToDirection = (key) => value(key).pipe(withReturnType(), when("ArrowUp", () => "Up"), when("ArrowDown", () => "Down"), when("ArrowLeft", () => "Left"), when("ArrowRight", () => "Right"), option);
...
var menuViewImpl = defineView((model, viewInputs) => {
	...
	const handleButtonKeyDown = (key) => {
		...
		return value(key).pipe(whenOr("Enter", " ", "ArrowDown", () => some(Opened({ maybeActiveItemIndex: some(firstEnabledIndex) }))), when("ArrowUp", () => some(Opened({ maybeActiveItemIndex: some(lastEnabledIndex) }))), orElse$1(() => none()));
	};
	...
	const handleItemsKeyDown = (key) => value(key).pipe(when("Escape", () => some(Closed())), when("Enter", () => map(maybeActiveItemIndex, (index) => RequestedItemClick({ index }))), when(" ", () => isNonEmpty(searchQuery) ? searchForKey(" ") : map(maybeActiveItemIndex, (index) => RequestedItemClick({ index }))), whenOr("ArrowDown", "ArrowUp", "Home", "End", "PageUp", "PageDown", () => some(ActivatedItem({
		index: resolveActiveIndex(key),
		activationTrigger: "Keyboard"
	}))), when(isPrintableKey, () => searchForKey(key)), orElse$1(() => none()));
...
var internalView = defineView((model, viewInputs) => {
	...
	const { nextKey, previousKey } = value(orientation).pipe(when("Horizontal", () => ({
		nextKey: "ArrowRight",
	...
	}), when(" ", () => pipe(options, get(currentIndex), map((value) => SelectedOption({
		value,
		index: currentIndex
	})))), orElse$1(() => none()));
...
var view$3 = defineView((model, viewInputs) => {
	...
	const handleKeyUp = (key) => value(key).pipe(when(" ", () => some(Toggled())), orElse$1(() => none()));
...
var variantToRole = (variant) => value(variant).pipe(withReturnType(), when("Info", () => "status"), when("Success", () => "status"), when("Warning", () => "alert"), when("Error", () => "alert"), exhaustive);
var positionToContainerStyle = (position) => {
	...
	return value(position).pipe(withReturnType(), when("TopLeft", () => ({
		...base,
		top: "0",
		left: "0",
...
var view$1 = defineView((model, viewInputs) => {
	...
	const handleTriggerKeyDown = (key) => value(key).pipe(when("Escape", () => when$2(isOpen, PressedEscape())), orElse$1(() => none()));
