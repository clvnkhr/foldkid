...
import { _ as identity, p as dual } from "./Function-CAyx_go8.js";
import { asEquivalence } from "./effect_Equal.js";
import { n as make } from "./Reducer-Cm_A1Upl.js";
import { isSome, none, some as some$1 } from "./effect_Option.js";
import { isFailure, isSuccess } from "./effect_Result.js";
//#region node_modules/effect/dist/Record.js
/**
...
var Record_exports = /* @__PURE__ */ __exportAll({
	...
	makeReducerIntersection: () => makeReducerIntersection,
	makeReducerUnion: () => makeReducerUnion,
	map: () => map,
...
* @since 2.0.0
*/
var get = /*#__PURE__*/ dual(2, (self, key) => has(self, key) ? some$1(self[key]) : none());
/**
...
* @since 2.0.0
*/
var replace = /*#__PURE__*/ dual(3, (self, key, b) => modify(self, key, () => b));
/**
...
* @since 2.0.0
*/
var pop = /*#__PURE__*/ dual(2, (self, key) => has(self, key) ? some$1([self[key], remove(self, key)]) : none());
/**
...
var filterMap = /*#__PURE__*/ dual(2, (self, f) => {
	...
	for (const key of keys(self)) {
		...
		if (isSuccess(result)) out[key] = result.success;
	}
	return out;
});
/**
...
var getSomes = (self) => {
	...
	for (const key of keys(self)) {
		...
		if (isSome(option)) out[key] = option.value;
	}
	return out;
};
/**
...
* @since 2.0.0
*/
var getFailures = (self) => {
	const out = empty();
	for (const key of keys(self)) {
		const value = self[key];
		if (isFailure(value)) out[key] = value.failure;
	}
	return out;
};
/**
...
* @since 2.0.0
*/
var getSuccesses = (self) => {
	const out = empty();
	for (const key of keys(self)) {
		const value = self[key];
		if (isSuccess(value)) out[key] = value.success;
	}
	return out;
};
/**
...
var partition = /*#__PURE__*/ dual(2, (self, f) => {
	...
	for (const key of keys(self)) {
		...
		if (isFailure(e)) left[key] = e.failure;
		else right[key] = e.success;
	}
	return [left, right];
});
/**
...
* @since 2.0.0
*/
var separate = /*#__PURE__*/ partition(identity);
/**
...
* @since 2.0.0
*/
var isSubrecord = /*#__PURE__*/ isSubrecordBy(/*#__PURE__*/ asEquivalence());
/**
...
var every = /*#__PURE__*/ dual(2, (self, refinement) => {
	for (const key of keys(self)) if (!refinement(self[key], key)) return false;
	return true;
});
/**
...
var some = /*#__PURE__*/ dual(2, (self, predicate) => {
	for (const key of keys(self)) if (predicate(self[key], key)) return true;
	return false;
});
/**
...
* @since 2.0.0
*/
var union = /*#__PURE__*/ dual(3, (self, that, combine) => {
	if (isEmptyRecord(self)) return { ...that };
	if (isEmptyRecord(that)) return { ...self };
	const out = empty();
...
* @since 2.0.0
*/
var intersection = /*#__PURE__*/ dual(3, (self, that, combine) => {
	const out = empty();
...
* @since 2.0.0
*/
var makeEquivalence = (equivalence) => {
...
* @since 2.0.0
*/
var singleton = (key, value) => ({ [key]: value });
/**
...
* @since 4.0.0
*/
function makeReducerUnion(combiner) {
	return make((self, that) => union(self, that, combiner.combine), {});
}
/**
...
* @since 4.0.0
*/
function makeReducerIntersection(combiner) {
	return make((self, that) => intersection(self, that, combiner.combine), {});
}
/**
...
var findFirst = /*#__PURE__*/ dual(2, (self, f) => {
	...
	for (let i = 0; i < k.length; i++) {
		const key = k[i];
		if (f(self[key], key)) return some$1([key, self[key]]);
	}
	return none();
});
//#endregion
export { collect, difference, empty, every, filter, filterMap, findFirst, fromEntries, fromIterableBy, fromIterableWith, get, getFailures, getSomes, getSuccesses, has, intersection, isEmptyReadonlyRecord, isEmptyRecord, isSubrecord, isSubrecordBy, keys, makeEquivalence, makeReducerIntersection, makeReducerUnion, map, mapEntries, mapKeys, modify, partition, pop, reduce, remove, replace, separate, set, singleton, size, some, Record_exports as t, toEntries, union, values };

//# sourceMappingURL=effect_Record.js.map